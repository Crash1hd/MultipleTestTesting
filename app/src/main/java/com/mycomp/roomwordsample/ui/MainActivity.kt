...
import com.mycomp.roomwordsample.R
...
class MainActivity : AppCompatActivity(), KodeinAware {
    ...
    private val wordViewModelFactory: WordViewModelFactory by instance()
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == newWordActivityRequestCode && resultCode == Activity.RESULT_OK) {
            data?.let {
                val word = listOf(Word(it.getStringExtra(NewWordActivity.EXTRA_REPLY)))
                val foo = wordViewModel.insertAll(word)
